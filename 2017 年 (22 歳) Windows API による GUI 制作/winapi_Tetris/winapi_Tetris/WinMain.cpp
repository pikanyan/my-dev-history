		...
		if (i % 3 == 0)
		{
			...
			if (!paintFallingTrn())
			{
				...
				for(int BoardSqyPos = 1; BoardSqyPos <= BoardVisibleSqyNum; BoardSqyPos++)
				{
					...
					{
						if (Board[BoardSqxPos][BoardSqyPos] == N)
							flag = false;
					}

					if (flag)
					{
						for (int i = BoardSqyPos; i <= BoardVisibleSqyNum; i++)
						{
							for (int BoardSqxPos = 1; BoardSqxPos <= BoardVisibleSqxNum; BoardSqxPos++)
							{
								Board[BoardSqxPos][i] = Board[BoardSqxPos][i + 1];
							}
						}
						BoardSqyPos--;
					}
				}
				if (!paintFallingTrnDefPos())
				{
					KillTimer(hWnd, 0);
